-- 1) Novo vocabulário de status das tarefas de missão
ALTER TABLE public.agitation_tasks DROP CONSTRAINT IF EXISTS agitation_tasks_status_check;

UPDATE public.agitation_tasks SET status = 'sem_acao'       WHERE status = 'pending';
UPDATE public.agitation_tasks SET status = 'pendente_envio' WHERE status = 'nao_enviado';
UPDATE public.agitation_tasks SET status = 'enviado'        WHERE status = 'concluido';
UPDATE public.agitation_tasks SET status = 'arquivado_erro' WHERE status = 'erro_numero';

-- Opt-out passa a ser legível na própria tarefa
UPDATE public.agitation_tasks t
   SET status = 'arquivado_optout'
  FROM public.contacts c
 WHERE c.id = t.contact_id
   AND c.opt_out_at IS NOT NULL
   AND t.status IN ('sem_acao', 'pendente_envio');

ALTER TABLE public.agitation_tasks ALTER COLUMN status SET DEFAULT 'sem_acao';
ALTER TABLE public.agitation_tasks
  ADD CONSTRAINT agitation_tasks_status_check
  CHECK (status IN ('sem_acao','pendente_envio','enviado','arquivado_erro','arquivado_optout'));

-- 2) Funções que dependiam de 'pending'
...
-- Liberar atribuições paradas: só toca em tarefas SEM AÇÃO (nunca em arquivadas)
