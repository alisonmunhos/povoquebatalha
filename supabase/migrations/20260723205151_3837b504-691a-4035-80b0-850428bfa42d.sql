-- Consentimento LGPD (campo independente do WhatsApp) + link opcional em perguntas de formulário.

-- Bloco A: LGPD em contacts
...
CREATE INDEX IF NOT EXISTS idx_contacts_consentimento_lgpd
  ON public.contacts(consentimento_lgpd)
...
-- Bloco B: link opcional em perguntas
...
DO $$ BEGIN
  ALTER TABLE public.form_definition_questions
    ADD CONSTRAINT form_definition_questions_link_pair_check CHECK (
      (link_text IS NULL AND link_url IS NULL)
      OR (
        link_text IS NOT NULL AND btrim(link_text) <> ''
        AND link_url IS NOT NULL AND btrim(link_url) <> ''
      )
    );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Bloco C: merge_contacts reconhece consentimento_lgpd
