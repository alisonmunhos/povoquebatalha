-- Consentimento para Tratamento de Dados Pessoais Sensíveis (LGPD Art. 5º, II).
-- Espelha a arquitetura de consentimento_lgpd — independente de WhatsApp e LGPD geral.

ALTER TABLE public.contacts
  ADD COLUMN IF NOT EXISTS consentimento_dados_sensiveis boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS consentimento_dados_sensiveis_at timestamptz;

CREATE INDEX IF NOT EXISTS idx_contacts_consentimento_dados_sensiveis
  ON public.contacts(consentimento_dados_sensiveis)
...
AS $$
BEGIN
  IF NEW.consentimento_whatsapp IS TRUE
     AND (TG_OP = 'INSERT' OR OLD.consentimento_whatsapp IS DISTINCT FROM TRUE) THEN
    IF NEW.consentimento_at IS NULL THEN NEW.consentimento_at := now(); END IF;
  END IF;
  IF NEW.consentimento_lgpd IS TRUE
     AND (TG_OP = 'INSERT' OR OLD.consentimento_lgpd IS DISTINCT FROM TRUE) THEN
    IF NEW.consentimento_lgpd_at IS NULL THEN NEW.consentimento_lgpd_at := now(); END IF;
  END IF;
  IF NEW.consentimento_dados_sensiveis IS TRUE
     AND (TG_OP = 'INSERT' OR OLD.consentimento_dados_sensiveis IS DISTINCT FROM TRUE) THEN
    IF NEW.consentimento_dados_sensiveis_at IS NULL THEN NEW.consentimento_dados_sensiveis_at := now(); END IF;
  END IF;
  RETURN NEW;
END $$;
...
CREATE OR REPLACE FUNCTION public.merge_contacts(
  ...
  p_confianca text DEFAULT NULL::text
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_field_overrides IS NOT NULL AND jsonb_typeof(p_field_overrides) = 'object' THEN
    FOR v_key, v_val IN SELECT key, value::text FROM jsonb_each_text(p_field_overrides) LOOP
      ...
      ELSIF v_key = 'consentimento_lgpd' THEN
        v_updates := v_updates || jsonb_build_object('consentimento_lgpd', (v_val::boolean));
      ELSIF v_key = 'consentimento_dados_sensiveis' THEN
        v_updates := v_updates || jsonb_build_object('consentimento_dados_sensiveis', (v_val::boolean));
      END IF;
    END LOOP;
  END IF;
