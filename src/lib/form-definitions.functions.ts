  ...
  .handler(async ({ data, context }) => {
    ...
    if (srcLayoutMode === "sectioned") {
      const { data: sections, error: secErr } = await context.supabase
        .from("form_sections")
        .select(
          "id,order_index,title,section_type,account_creation_role,default_next_section_id,confirmation_active,whatsapp_button_enabled,whatsapp_button_message,success_screen_order",
        )
        .eq("form_definition_id", data.id)
        .order("order_index", { ascending: true });
