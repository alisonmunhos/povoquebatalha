...
function CadastroAgitador() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      const r = await fetch("/api/public/forms/cadastro-agitador", {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-muted/20">
      ...
      <main className="max-w-md mx-auto px-6 py-10">
        {success ? (
          <SuccessScreen state={success} />
        ) : (
          <>
            <h1 className="text-3xl font-bold tracking-tight">Cadastro de agitador(a)</h1>
            <p className="mt-2 text-muted-foreground">
              Preencha seus dados para se cadastrar como agitador(a) voluntário(a). Um administrador aprovará seu acesso.
            </p>
...
function SuccessScreen({ state }: { state: SuccessState }) {
  ...
  const waMsg = encodeURIComponent(
    "Olá Campanha do Povo que Batalha! Finalizei meu cadastro de agitador. Aguardo aprovação.",
  );
